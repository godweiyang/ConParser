...
'''
TopDown + label-LSTM(input:label,lr_encoding) + span encoding=[lstm(l, r); label-LSTM output]
'''


class MyTopDownParser5(object):
    def network_init(self):
        ...
        self.label_lstm = dy.VanillaLSTMBuilder(
            2, self.label_embedding_dim + 4 * self.lstm_dim, self.lstm_dim, self.model)
    ...
    def parse(self, data):
        ...
        def helper(left, right, parent_state, direction):
            label_scores = get_label_scores(
                left, right, parent_state, direction)
            ...
            label_scores = MyTopDownParser5.augment(
            ...
            split_scores = MyTopDownParser5.augment(
            ...
            left_encoding = get_span_encoding(left, split - 1)
            right_encoding = get_span_encoding(split, right)
            child_state = parent_state.add_input(dy.concatenate(
                [self.label_embeddings[argmax_label_index], left_encoding, right_encoding]))

            left_trees, left_loss = helper(left, split - 1, child_state, 0)
            right_trees, right_loss = helper(split, right, child_state, 1)
        ...
        root_state = self.label_lstm.initial_state()
        root_encoding = get_span_encoding(0, len(sentence) - 1)
        root_state = root_state.add_input(dy.concatenate(
            [self.label_embeddings[0], root_encoding, root_encoding]))
        childrens, loss = helper(0, len(sentence) - 1, root_state, 0)
